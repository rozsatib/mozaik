...
def directives_from_dryrun(dry_line):
    ...
    flags = [
        t
        for t in toks[1:]
        if not t.endswith("run_job.sh") and not t.startswith("--export")
    ]
...
def capture_apptainer(cmd, extra_env, array_id, ntasks):
    ...
    try:
        ...
        if r.returncode == 77:
            sys.exit(
                f"ABORT: apptainer stub function not in effect — refusing to run (real-pipeline risk).\n{r.stderr.decode()}"
            )
        argv = [l.rstrip("\n") for l in open(cap)]
        return argv or None
    finally:
...
def capture_new(sc):
    ...
    dry = subprocess.run(
        ["bash", os.path.join(CL, "submit.sh"), confpath, "--dry-run"],
        capture_output=True,
        text=True,
        cwd=MOZAIK,
    )
    ...
    argv = capture_apptainer(
        f"MOZAIK_CONF={confpath} bash {os.path.join(CL,'run_job.sh')}", {}, aid, nt
    )
    return {"directives": directives, "apptainer_argv": argv}
...
def main():
    ...
    if args.out:
        ...
        for lbl, r in result.items():
            ...
            print(
                f"  {lbl:15} directives={len(r['directives'])} apptainer_argv_lines={n}"
            )
        return 0
    ...
    if args.cmp:
        ...
        print(
            "\nGATE:",
            (
                "PASS — all scenarios byte-identical"
                if ok
                else "FAIL — launch behavior changed"
            ),
        )
